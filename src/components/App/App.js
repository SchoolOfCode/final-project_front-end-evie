...
import Feedback from "../App/form/Form";
    ...
    geolocate.on("geolocate", function (ev) {
      ...
      async function Fetch() {
        ...
        data.forEach((location) => {
          // eslint-disable-next-line
          var marker = new mapboxgl.Marker({
            color: backendData[random].colour,
          })
            .setLngLat([
              location.AddressInfo.Longitude,
              location.AddressInfo.Latitude,
            ...
            .setPopup(
              new mapboxgl.Popup({ offset: 30 }).setHTML(
                "<h4>" +
                  ...
                  location.AddressInfo.Postcode +
                  "<h4>" +
                  backendData[random].FormalName +
                  "<h4>" +
                  backendData[random].Title +
                  "<h4>" +
                  backendData[random].stars
              )
            )
            .addTo(map.current);
